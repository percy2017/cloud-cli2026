...
/**
 * Build a child-process env that overrides git's author/committer identity
 * with the values stored in the system-wide git config. Lets `git commit`
 * succeed even when the server process has no identity of its own (e.g. when
 * the user has set git config --global in a different shell, or has not set
 * it at all but we still want a useful error rather than a generic 500).
 */
function buildGitCommitEnv(commitEnvBase) {
  const identity = commitEnvBase && commitEnvBase.git_name && commitEnvBase.git_email
    ? commitEnvBase
    : null;
...
router.get('/status', async (req, res) => {
  ...
  try {
    ...
    // Case-insensitive match — `validateGitRepository` throws
    // "Not a git repository…" with a capital N, which a naive .includes() miss.
    const isMissingRepo = /not a git repository/i.test(error.message);
    res.json({
      error: isMissingRepo
        ? error.message
        : 'Git operation failed',
      details: isMissingRepo
        ? error.message
...
router.post('/initial-commit', async (req, res) => {
  ...
  try {
    ...
    // Create initial commit. Use the system-wide git identity (if any) as
    // env-var overrides so the commit succeeds without a per-repo or per-shell
    // git config, and so failures show the real stderr instead of a generic
    // "Command failed" message.
    const systemGitConfig = await getSystemGitConfig().catch(() => ({ git_name: null, git_email: null }));
    const commitEnv = buildGitCommitEnv(systemGitConfig);
    const { stdout } = await spawnAsync('git', ['commit', '-m', 'Initial commit'], { cwd: projectPath, env: commitEnv });
    ...
    // mode — "nothing to commit" lands on stdout, "Please tell me who you are"
    // lands on stderr, etc. Combine both so the regex below matches either.
    const stderrText = String(error?.stderr || '').trim();
    const stdoutText = String(error?.stdout || '').trim();
    const combined = `${error?.message || ''}\n${stderrText}\n${stdoutText}`.toLowerCase();
    const isNothingToCommit = combined.includes('nothing to commit');
    const isMissingIdentity = /please tell me who you are|unable to auto-detect email address/i.test(combined);
...
router.post('/init', async (req, res) => {
  ...
  try {
    ...
    // If it's already a repo, no-op success — avoids clobbering an existing repo.
    try {
  ...
  try {
    ...
    for (const file of files) {
      ...
      try {
        ...
        stagedFiles.push(repositoryRelativeFilePath);
      } catch (error) {
        const stderr = `${error?.stderr || ''}`;
        if (stderr.includes('did not match any files')) {
          console.warn(`Skipping commit for "${repositoryRelativeFilePath}": ${stderr.trim()}`);
          skippedFiles.push(repositoryRelativeFilePath);
          continue;
        }
        throw error;
      }
    ...
    if (stagedFiles.length === 0) {
      return res.status(400).json({
        error: 'No files could be staged for commit',
        details: skippedFiles.length > 0
          ? `All ${skippedFiles.length} file(s) were skipped (not found in working tree or ignored by .gitignore): ${skippedFiles.join(', ')}`
          : 'None of the selected files exist in the working tree.',
...
router.post('/publish', async (req, res) => {
  ...
  try {
    ...
    // Publish the branch (set upstream and push). For GitHub remotes, authenticate
    // using the user's stored Personal Access Token (same source /api/agent uses).
    // We swap the remote URL temporarily to embed the token, then restore it.
    validateRemoteName(remoteName);
    let originalRemoteUrl = null;
    try {
      const { stdout: urlOutput } = await spawnAsync('git', ['remote', 'get-url', remoteName], { cwd: projectPath });
      ...
      if (originalRemoteUrl.startsWith('https://github.com/')) {
        ...
        if (!token) {
          return res.status(400).json({
            ...
            details: 'Configure your GitHub Personal Access Token in Settings → API Settings → Credentials → GitHub Tokens before publishing to a GitHub repository.',
          });
        }
        const authedUrl = originalRemoteUrl.replace(/^https:\/\/github\.com\//, `https://${token}@github.com/`);
        await spawnAsync('git', ['remote', 'set-url', remoteName, authedUrl], { cwd: projectPath });
      }
    } catch (urlError) {
      // Couldn't read/replace remote URL — fall through and try the push with whatever auth the user has.
      console.warn(`Publish: could not inspect remote URL for "${remoteName}":`, urlError.message);
    }
    ...
    try {
      ...
      if (originalRemoteUrl) {
        await spawnAsync('git', ['remote', 'set-url', remoteName, originalRemoteUrl], { cwd: projectPath }).catch(() => {});
      }
    }
  } catch (error) {
