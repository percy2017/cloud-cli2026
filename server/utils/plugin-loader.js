...
function runBuildIfNeeded(dir, packageJsonPath, onSuccess, onError) {
  ...
  const buildProcess = spawn('npm', ['run', 'build'], {
    cwd: dir,
    // Force development mode for npm install/build so devDependencies
    // (typescript, vite, esbuild, etc.) are available. The server's
    // own NODE_ENV=production otherwise causes npm to skip devDeps
    // and the build fails with "tsc: not found" / similar.
    ...
    gitProcess.on('close', (code) => {
      ...
      if (fs.existsSync(packageJsonPath)) {
        const npmProcess = spawn('npm', ['install', '--ignore-scripts'], {
          cwd: tempDir,
          // See runBuildIfNeeded above for why NODE_ENV is forced to development.
          env: { ...process.env, NODE_ENV: 'development' },
          stdio: ['ignore', 'pipe', 'pipe'],
        });
