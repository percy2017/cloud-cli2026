...
export function startPluginServer(name, pluginDir, serverEntry) {
  ...
  const startPromise = new Promise((resolve, reject) => {
    ...
    // Use the same Node interpreter as the main server (NODE_BINARY) so plugin
    // native modules (e.g. node-pty in cloudcli-plugin-terminal) match the
    // server's ABI. Falls back to "node" from PATH when unset.
    const nodeBin = process.env.NODE_BINARY || 'node';
    const pluginProcess = spawn(nodeBin, [serverPath], {
      cwd: pluginDir,
