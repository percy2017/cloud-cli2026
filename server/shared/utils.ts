...
export async function validateWorkspacePath(requestedPath: string): Promise<WorkspacePathValidationResult> {
  try {
    ...
    // The configured workspace root itself is always allowed, even if its
    // resolved value happens to be a system path (e.g. "/" or "/root").
    const isWorkspaceRoot = normalizedPath === normalizedWorkspaceRoot;
    // When the admin pins WORKSPACES_ROOT=/ they have explicitly opted in to
    // unrestricted filesystem access; skip the FORBIDDEN_WORKSPACE_PATHS checks
    // entirely in that case. The containment check below still applies.
    const unrestrictedRoot = normalizedWorkspaceRoot === '/';

    if (!unrestrictedRoot && (FORBIDDEN_WORKSPACE_PATHS.includes(normalizedPath) || normalizedPath === '/') && !isWorkspaceRoot) {
      return {
        valid: false,
    ...
    if (!unrestrictedRoot) {
      for (const forbiddenPath of FORBIDDEN_WORKSPACE_PATHS) {
        ...
        if (
          ...
          // The configured workspace root itself is always allowed, even when
          // it matches a forbidden entry verbatim (e.g. WORKSPACES_ROOT=/).
          if (isWorkspaceRoot) {
            continue;
          }
    ...
    // When the workspace root is "/" the naive `${root}${sep}` prefix would
    // produce "//", which no real path starts with, so any subdirectory would
    // be wrongly rejected. Treat "/" as containing every absolute path.
