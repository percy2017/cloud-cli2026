...
export default function PluginSettingsTab() {
  ...
  // Accept either a full git URL or the GitHub `org/repo` shorthand and
  // normalize it to an `https://github.com/<org>/<repo>` URL the backend
  // will accept. Mirrors the prefix check in `server/routes/plugins.js`.
  const normalizePluginUrl = (raw: string): string => {
    const value = raw.trim();
    if (!value) return value;
    if (value.startsWith('https://') || value.startsWith('git@')) return value;
    // `org/repo` shorthand → assume github.com.
    const shorthandMatch = /^([\w.-]+)\/([\w.-]+?)(?:\.git)?$/;
    const m = value.match(shorthandMatch);
    if (m && !value.includes('://') && !value.includes('@')) {
  ...
  const handleInstall = async () => {
    const url = normalizePluginUrl(gitUrl);
    if (!url) return;
    setGitUrl(url);
    setInstalling(true);
  ...
  const handleInstallRecommendation = async (recommendation: PluginRecommendation) => {
    ...
    setInstallError(null);
    try {
      const url = normalizePluginUrl(recommendation.repoUrl);
      const result = await installPlugin(url);
      if (!result.success) {
        setInstallError(result.error || t('pluginSettings.installFailed'));
      }
