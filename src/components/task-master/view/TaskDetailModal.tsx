...
const STATUS_VALUES = ['pending', 'in-progress', 'review', 'done', 'deferred', 'cancelled'] as const;
...
function getStatusLabelKey(value: StatusValue): string {
  switch (value) {
    case 'pending':
      return 'detail.statuses.pending';
    case 'in-progress':
      return 'detail.statuses.inProgress';
    case 'review':
      return 'detail.statuses.review';
    case 'done':
      return 'detail.statuses.done';
    case 'deferred':
      return 'detail.statuses.deferred';
    case 'cancelled':
      return 'detail.statuses.cancelled';
  }
}
...
export default function TaskDetailModal({
  ...
  const statusOptions = useMemo(
    () =>
      STATUS_VALUES.map((value) => ({
        value,
        label: t(getStatusLabelKey(value)),
      })),
    [t],
  );
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorPayload.message ?? t('detail.failedToUpdate'));
      }
      ...
      alert(error instanceof Error ? error.message : t('detail.failedToUpdate'));
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleStatusSelect = async (nextStatus: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorPayload = (await response.json()) as { message?: string };
        throw new Error(errorPayload.message ?? t('detail.failedToUpdateStatus'));
      }
      ...
      alert(error instanceof Error ? error.message : t('detail.failedToUpdateStatus'));
    }
  };
          ...
          <div className="flex min-w-0 flex-1 items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <button
                ...
                title={t('detail.copyIdTooltip')}
              >
                <span>{t('detail.taskLabel', { id: task.id })}</span>
            ...
            {isEditMode ? (
              <>
                ...
                <button
                  ...
                  title={t('detail.cancelEditTooltip')}
                >
                  <X className="h-5 w-5" />
                </button>
              </>
              ...
              <button
                ...
                className="rounded-md p-2 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800"
                title={t('detail.editTaskTooltip')}
              >
                <Edit className="h-5 w-5" />
              </button>
            )}
            <button onClick={onClose} className="rounded-md p-2 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800" title={t('detail.closeTooltip')}>
              <X className="h-5 w-5" />
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 space-y-6 overflow-y-auto p-4 md:p-6">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('detail.statusLabel')}</label>
              <select
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('detail.priorityLabel')}</label>
              <div className={cn('px-3 py-2 rounded-md text-sm font-medium capitalize', getPriorityBadgeClass(task.priority))}>
                {task.priority ?? t('detail.notSet')}
              </div>
            </div>

            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('detail.dependenciesLabel')}</label>
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('detail.descriptionLabel')}</label>
            {isEditMode ? (
              <textarea
                rows={4}
