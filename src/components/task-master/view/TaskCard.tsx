...
function getStatusStyle(t: Translator, status?: string): TaskStatusStyle {
  if (status === 'done') {
    return {
      icon: CheckCircle,
      statusText: t('badges.done'),
  ...
  if (status === 'in-progress') {
    return {
      icon: Clock,
      statusText: t('badges.inProgress'),
  ...
  return {
    icon: Circle,
    statusText: t('badges.pending'),
    iconColor: 'text-slate-500 dark:text-slate-400',
    textColor: 'text-slate-900 dark:text-slate-100',
  };
}

function renderPriorityIcon(t: Translator, priority?: string) {
  if (priority === 'high') {
    return (
      <Tooltip content={t('priorityTooltip.high')}>
  ...
  if (priority === 'medium') {
    return (
      <Tooltip content={t('priorityTooltip.medium')}>
  ...
  if (priority === 'low') {
    return (
      <Tooltip content={t('priorityTooltip.low')}>
  ...
  return (
    <Tooltip content={t('priorityTooltip.none')}>
...
function TaskCard({ task, onClick = null, showParent = false, className = '' }: TaskCardProps) {
  ...
  const statusStyle = getStatusStyle(t, task.status);
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          ...
          {showParent && task.parentId && (
            <span className="text-xs font-medium text-gray-500 dark:text-gray-400">{t('nextTask.label', { id: task.parentId })}</span>
          )}
        </div>

        <div className="flex-shrink-0">{renderPriorityIcon(t, task.priority)}</div>
      </div>
          ...
          {Array.isArray(task.dependencies) && task.dependencies.length > 0 && (
            <Tooltip content={`Depends on: ${task.dependencies.map((dependency) => `Task ${dependency}`).join(', ')}`}>
              <div className="flex items-center gap-1 text-xs text-amber-600 dark:text-amber-400">
                ...
                <span>{t('card.dependsOn')} {task.dependencies.join(', ')}</span>
              </div>
            </Tooltip>
          )}
        </div>
      ...
      {progress.total > 0 && (
        <div className="ml-3">
          <div className="mb-1 flex items-center gap-2">
            <span className="text-xs text-gray-500 dark:text-gray-400">{t('card.progressLabel')}</span>
